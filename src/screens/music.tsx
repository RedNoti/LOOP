...
export default function YouTubeMusicPlayer({
  ...
  // ===== [2025-05-31 수행] 단일 영상 재생 UI 업데이트를 위한 이벤트 리스너 복원 =====
  useEffect(() => {
    const handleUpdateCurrentVideo = (event: any) => {
      const { videoId, title, thumbnail, index } = event.detail;
      
      console.log(`🎵 music.tsx에서 단일 영상 이벤트 수신: ${title} (ID: ${videoId})`);
      
      // sessionStorage에서 최신 상태 읽어오기
      const updatedVideos = JSON.parse(sessionStorage.getItem("musicPlayerVideos") || "[]");
      const updatedCurrentVideoId = sessionStorage.getItem("currentVideoId");
      
      if (updatedVideos.length > 0 && updatedCurrentVideoId) {
        setVideos(updatedVideos);
        console.log(`✅ music.tsx UI 업데이트 완료: ${title}`);
      }
    };

    window.addEventListener("updateCurrentVideo", handleUpdateCurrentVideo);
    return () => {
      window.removeEventListener("updateCurrentVideo", handleUpdateCurrentVideo);
    };
  }, [setVideos]);
