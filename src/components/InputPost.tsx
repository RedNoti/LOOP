  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (attachPlaylist && currentPlaylistId && attachedPlaylist) {
      try {
        ...
        const response = await fetch(
          "https://loopmusic.o-r.kr:4003/upload/playlist",
          {
            method: "POST",
            body: formData,
          }
        ...
        if (data.success && data.data && data.data.filename) {
          playlistFileUrl = `https://loopmusic.o-r.kr:4003/uploads/shared_playlists/${data.data.filename}`;
        }
      } catch (err) {
    ...
    try {
      for (const file of files) {
        ...
        const response = await fetch(
          "https://loopmusic.o-r.kr:4003/upload/post",
          {
            method: "POST",
            body: formData,
          }
        ...
        if (data.success && data.data && data.data.filename) {
          const imageUrl = `https://loopmusic.o-r.kr:4003/uploads/post_images/${data.data.filename}`;
